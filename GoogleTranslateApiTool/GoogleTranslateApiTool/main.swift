...
func getSound(for word: String, lang: String) async -> Data?  {
    ...
    try! data.write(to: URL(fileURLWithPath: "data.wad"))
    let str = data.split(separator: Character(",").asciiValue!)[2]
    try! str.write(to: URL(fileURLWithPath: "str.wad"))
        ...
        if line.contains("rPsWke".utf8) {
            ...
            for component in subobj4 {
                ...
                for subcomponent in subobj6 {
                    ...
                    result.append(.init(word: translation, freq: freq, type: EWordType(rawValue: wordType)!))
                }
...
//let data = await getSound(for: "नमस्ते", lang: "hi")!
//try! data.write(to: URL(fileURLWithPath: "out.mp3"))
let details = await getTranslation(for: "चश्मा", lang: "hi")
print("Details: ", details)
print(details[0].type.toString())
